...
fn gzip_round_trip_matches_known_content() {
    ...
    assert!(
        ...
        "Should contain database element"
    );
    assert!(
        content.contains("</database>"),
        "Should close database element"
    );
    ...
    assert!(
        ...
        "Should have families section"
    );
    assert!(content.contains("<header>"), "Should have header section");

    // Verify specific content.
    assert!(
        content.contains("Harry"),
        "Should contain known person: Harry"
    );
    assert!(
        content.contains("Meowser"),
        "Should contain surname: Meowser"
    );
    assert!(content.contains("Sally"), "Should contain person: Sally");
    assert!(
        content.contains("Furball"),
        "Should contain surname: Furball"
    );
...
fn gzip_counts_match_expected() {
    ...
    assert_eq!(
        report.people_not_in_family, 0,
        "All people should be in a family"
    );
...
fn gzip_graph_data_populated_correctly() {
    ...
    assert!(
        family.father_handle.is_some(),
        "Family should have a father"
    );
    assert!(
        family.mother_handle.is_some(),
        "Family should have a mother"
    );
