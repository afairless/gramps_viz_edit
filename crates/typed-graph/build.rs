//! Build script — reads `schema.json` and generates Rust types at compile time.
//!
//! # How it works
//!
//! 1. Locates `schema.json` relative to the workspace root (via `CARGO_MANIFEST_DIR`).
...
fn main() {
    ...
    let schema: serde_json::Value = match serde_json::from_str(&schema_json) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("error: failed to parse schema.json: {}", e);
            std::process::exit(1);
        }
    };
...
fn generate_code(schema: &serde_json::Value) -> String {
    ...
    code.push_str("// This file is @generated by build.rs from schema.json.\n");
    code.push_str("// Do not edit manually — edit schema.json and rebuild.\n\n");
