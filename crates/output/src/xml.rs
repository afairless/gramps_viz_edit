...
impl GraphXmlWriter {
    ...
    /// Set a researcher note for the header (e.g. "Cleaned by gramps-gen delete").
    pub fn with_researcher_note(mut self, note: &str) -> Self {
        self.researcher_note = Some(note.to_string());
        self
    }
    ...
    fn write_header(&self, writer: &mut impl std::io::Write) -> Result<(), SerializationError> {
        ...
        writeln!(
            writer,
            "      <resname>{}</resname>",
            self.researcher_note.as_deref().unwrap_or("Generated by gramps-gen")
        )?;
        writeln!(writer, "    </researcher>")?;
        writeln!(writer, "  </header>")?;
        Ok(())
    ...
    fn write_edge_items(
        ...
        let matching_edges: Vec<&Edge> = edges_from
            .iter()
            .filter(|e| {
                edge_variant_name(e) == Some(edge_name)
                    && !self.is_edge_deleted(e)
            })
            .copied()
            .collect();
